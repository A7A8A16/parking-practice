...
- 2026-02-13: **v1.0.1 - 马年春节特别版**
  ...
  - **背景音乐系统**：
    ...
    - 返回菜单时重新播放
  - 技术实现：
    - 在 `Game.js` 和 `Menu.js` 中调用 `menu.show()` 触发效果
    - 彩带效果定义在 `index.html` 中的内联脚本
    - 添加调试日志便于排查

- 2026-02-13: **v1.0.2 - 彩带效果优化**
  - **祝福语霓虹灯效果**：
    - 字体加大到 2rem，加粗 700
    - 彩虹渐变色自动流动动画
    - 多层发光效果模拟霓虹灯管
  - **彩带效果大幅增强**：
    ...
    - 下落时间：2-4秒 → 2.1-5.1秒
    - 旋转圈数：最多 4 圈 → 6 圈
    - 形状多样化：30% 正方形彩纸 + 70% 长条彩带
    - 动画关键帧优化：7 个关键帧实现更自然的运动曲线
    - 使用 cubic-bezier 缓动函数模拟重力效果
  - **修复关键 bug**：
    - 修复多个 CSS 动画冲突导致彩带不动的问题
    - 修复 `Game.js` 未调用 `menu.show()` 导致音乐不播放的问题
  - 视觉效果提升：彩带更大更明显，飘散效果更震撼
